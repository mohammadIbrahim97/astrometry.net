...
#define SOLVER_CODEKD_DELIVERY_BUDGET_BYTES \
    (2U * 1024U * 1024U)
#define SOLVER_CANDIDATE_DELIVERY_LIMIT 128U
/* Sweep batches stop at the existing byte/range budgets if the window is dense. */
#define SOLVER_VERIFY_QUERY_LOOKAHEAD \
    SOLVER_CANDIDATE_DELIVERY_LIMIT
#define SOLVER_CANDIDATE_STAR_LIMIT \
    (SOLVER_CANDIDATE_DELIVERY_LIMIT * DQMAX)
#if SOLVER_CANDIDATE_STAR_LIMIT > FITSBIN_PREFETCH_RANGE_LIMIT
...
typedef struct solver_codekd_page_hash_slot {
    size_t index;
    unsigned int generation;
} solver_codekd_page_hash_slot_t;

typedef struct solver_codekd_page_set {
    solver_codekd_page_entry_t* entries;
    solver_codekd_page_hash_slot_t* hash;
    size_t count;
    size_t capacity;
...
typedef struct solver_codekd_phase_context {
    solver_ab_descriptor_phase_context_t descriptor;
    const kdtree_t* tree;
